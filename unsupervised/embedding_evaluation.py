...
class EmbeddingEvaluation():
    ...
    def ee_binary_classification(self, train_emb, train_y, val_emb, val_y, test_emb, test_y):
        if self.param_search:
            ...
            inner_cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=self.seed)
            # StandardScaler must be refit inside each inner CV fold, not once on the
            # whole training set beforehand -- otherwise every inner-validation
            # subject's own values already contributed to the scaling statistics
            # applied to it, leaking information into the C search.
        ...
        if self.eval_metric == 'accuracy':
            ...
            # AUC needs a continuous, threshold-independent score -- hard 0/1
            # predictions collapse roc_auc_score into a rescaled duplicate of
            # balanced accuracy. LinearSVC/SVC (no probability=True) have no
            # predict_proba, but do expose decision_function for exactly this.
    ...
    def embedding_evaluation(self, encoder, beta, train_loader, valid_loader, test_loader, flag):
        ...
        if 'classification' in self.task_type:
            ...
            if self.num_tasks == 1:
                train_raw, val_raw, test_raw, test_auc_raw = self.ee_binary_classification(
                    train_emb, train_y, val_emb, val_y, test_emb, test_y)
