...
**Issues #6-8 — signed PCC used directly as the GCN's edge weight.** The paper defines two distinct quantities: `A`, the learned adjacency (Eq. 12: `A = σ(...)`, always positive, initialized fully connected per Appendix A.1), and `W`, the raw signed Pearson correlation. The released code never separated them — it fed the signed PCC straight into `GCNConv`'s edge weight, and negative correlations produced negative weighted node degrees, causing NaNs. Fixed by making the graph genuinely fully-connected (all N×N pairs, explicit self-loops) with the signed PCC preserved only as `ToyNet`'s VIB input, while the GCN itself now receives `edge_weight=None` (uniform, unweighted `A`) for the original view and a sigmoid-gated, always-positive weight for the augmented view. Verified: constructing the real `GCNConv` with signed PCC fed directly reproduces the NaN bug exactly; the fix produces finite output under identical input.
