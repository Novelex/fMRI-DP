#!/bin/bash
#SBATCH --job-name=ml-combat-gpcc
...
# 7 algorithms, global_pcc feature set only, ComBat-harmonized -- one per array task.
ALGORITHMS=(elasticnet linear_svm rbf_svm random_forest gradient_boosting knn mlp)

ALGORITHM=${ALGORITHMS[$SLURM_ARRAY_TASK_ID]}

echo "Task ${SLURM_ARRAY_TASK_ID}: algorithm=${ALGORITHM} feature-set=global_pcc_combat"

cd "${PROJECT_ROOT}"

"${PYTHON}" ml_combat_gPCC/run_baseline_combat.py \
  --algorithm "${ALGORITHM}" \
  --out-dir "${PROJECT_ROOT}/ml_combat_gPCC/results"
