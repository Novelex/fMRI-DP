"""Classical ML baseline on COMBINED feature sets, with ComBat site harmonization.

Same 7 algorithms and nested 5-fold stratified CV as ml/run_baseline.py and the
single-feature-set ml_combat_gPCC/ml_combat_dPCC scripts, but the feature
vector is now a concatenation of two or three of {global_pcc, local_pcc,
alff} per subject, selected via --feature-combo. ComBat harmonization (site=
batch, age+sex=covariates, diagnosis excluded -- "No Target" scheme) is
applied to the combined vector exactly as before: it corrects each feature
column independently, so mixing PCC-scale and ALFF-scale columns in one
matrix is structurally fine -- the StandardScaler immediately downstream of
it (inside the same cross-validated Pipeline) handles the remaining
scale differences between feature families.

Usage:
    python run_baseline_combat.py --algorithm elasticnet --feature-combo gpcc_lpcc
"""
import argparse
import json
import os
...
FEATURE_COMBOS = {
    "gpcc_lpcc": ["global_pcc", "local_pcc"],
    "gpcc_alff": ["global_pcc", "alff"],
    "lpcc_alff": ["local_pcc", "alff"],
    "all3": ["global_pcc", "local_pcc", "alff"],
}


# --------------------------------------------------------------------------- #
# Data loading -- loads all three raw feature types per subject in one pass
# (file-matching-by-ID, never os.listdir() position -- see ISSUES.md), plus
# SITE/AGE/SEX covariates joined by FILE_ID. The requested combination is
# assembled afterward by concatenating the relevant pieces.
# --------------------------------------------------------------------------- #
...
def _load_group(adj_dir, nf_dir, dw_dir, y):
    ...
    global_rows, local_rows, alff_rows = [], [], []
    for sid in subject_ids:
        adj_mat = sio.loadmat(osp.join(adj_dir, adj_files[sid]))
        pcc = np.nan_to_num(adj_mat["cropped_matrix"])
        global_rows.append(_upper_tri(pcc))

        dw_mat = sio.loadmat(osp.join(dw_dir, dw_files[sid]))
        dw_array = dw_mat["correlation_matrices"]
        ...
        local_rows.append(np.concatenate([mean_conn, std_conn]))

        nf_mat = sio.loadmat(osp.join(nf_dir, nf_files[sid]))
        nf = np.nan_to_num(nf_mat["norm_matrix"])
        alff_rows.append(nf.flatten())

    X_global = np.stack(global_rows)
    X_local = np.stack(local_rows)
    X_alff = np.stack(alff_rows)
    y_arr = np.full(len(subject_ids), y, dtype=np.int64)
    return X_global, X_local, X_alff, y_arr, subject_ids


def load_features_and_covars():
    Xg_asd, Xl_asd, Xa_asd, y_asd, ids_asd = _load_group(
        osp.join(DATA_ROOT, "ASD_ADJ"), osp.join(DATA_ROOT, "ASD_NF"), osp.join(DATA_ROOT, "ASD_DW"), y=1,
    )
    Xg_nc, Xl_nc, Xa_nc, y_nc, ids_nc = _load_group(
        osp.join(DATA_ROOT, "NC_ADJ"), osp.join(DATA_ROOT, "NC_NF"), osp.join(DATA_ROOT, "NC_DW"), y=0,
    )
    X_global = np.concatenate([Xg_asd, Xg_nc], axis=0)
    X_local = np.concatenate([Xl_asd, Xl_nc], axis=0)
    X_alff = np.concatenate([Xa_asd, Xa_nc], axis=0)
    ...
    return {"global_pcc": X_global, "local_pcc": X_local, "alff": X_alff}, y, covars


def build_combo(feature_dict, combo):
    parts = [feature_dict[name] for name in FEATURE_COMBOS[combo]]
    return np.concatenate(parts, axis=1)


# --------------------------------------------------------------------------- #
# ComBat as an sklearn-compatible Pipeline step -- identical mechanism to
# ml_combat_gPCC/ml_combat_dPCC. It corrects each feature column
# independently, so a combined vector spanning multiple feature families
...
def harmonization_apply_safe(feats, covars, model):
    """See ml_combat_gPCC/run_baseline_combat.py for the full explanation:
    ...
    learned during training, so a CV fold missing one site crashes with a
    matmul shape mismatch. Padding with one dummy row per missing site
    (discarded after transform) fixes the shape without affecting real rows,
    since adjust_data_final processes each site's rows independently.
    """
...
# function's StandardScaler is already handled in ml/run_baseline.py.
# --------------------------------------------------------------------------- #
...
# inside one Pipeline that GridSearchCV cross-validates.
# --------------------------------------------------------------------------- #
...
def main():
    ...
    parser.add_argument("--feature-combo", required=True, choices=list(FEATURE_COMBOS.keys()))
    parser.add_argument("--out-dir", default="/users/3171356m/muhammad/GraSTIACL/ml_combat_combinations/results")
    args = parser.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)
    np.random.seed(SEED)

    print(f"Loading feature sets for combo: {args.feature_combo} = {FEATURE_COMBOS[args.feature_combo]}")
    feature_dict, y, covars = load_features_and_covars()
    X = build_combo(feature_dict, args.feature_combo)
    ...
    result = {
        ...
        "feature_set": f"{args.feature_combo}_combat",
        "n_subjects": int(len(y)),
        "n_features": int(X.shape[1]),
        "fold_scores": fold_scores,
        "summary": summary,
    }
    out_path = osp.join(args.out_dir, f"{args.algorithm}__{args.feature_combo}_combat.json")
