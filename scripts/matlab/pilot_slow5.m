% Pilot ALFF run: same subject (SDSU_0050182), slow-5 band (0.010-0.027 Hz).
...
work_dir = fullfile(project_root, 'data/dparsf_work/pilot_slow5');
...
config_file = fullfile(project_root, 'data/dparsf_work/pilot_configs/Cfg_slow5_pilot.mat');
...
Cfg.CalALFF.ALowPass_HighCutoff = 0.027;

save(config_file, 'Cfg');
disp('Pilot slow-5 config saved.');

[Error, AutoDataProcessParameter] = DPARSFA_run(Cfg, work_dir, subject_list_file, 0);

result_file = fullfile(project_root, 'data/dparsf_work/pilot_configs/Slow5_pilot_result.mat');
