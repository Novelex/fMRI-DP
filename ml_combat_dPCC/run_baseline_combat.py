"""Classical ML baseline on Dynamic (Local) PCC features, with ComBat site harmonization.

Same 7 algorithms and nested 5-fold stratified CV as ml/run_baseline.py, but
restricted to the local_pcc (dynamic/windowed PCC, mean+std across windows)
feature set, with a ComBat harmonization step (site=batch, age+sex=covariates,
diagnosis excluded -- "No Target" scheme) fit fresh inside every training
split so no test subject's data ever contributes to the harmonization model.
...
# --------------------------------------------------------------------------- #
# Data loading -- local_pcc (dynamic PCC) only, file-matching-by-ID (never
# os.listdir() position -- see ISSUES.md), plus SITE/AGE/SEX covariates
# joined by FILE_ID.
# --------------------------------------------------------------------------- #
...
def main():
    ...
    parser.add_argument("--out-dir", default="/users/3171356m/muhammad/GraSTIACL/ml_combat_dPCC/results")
    args = parser.parse_args()

    os.makedirs(args.out_dir, exist_ok=True)
    np.random.seed(SEED)

    print("Loading feature set: local_pcc / dynamic PCC (+ SITE/AGE/SEX covariates)")
    ...
    result = {
        ...
        "feature_set": "local_pcc_combat",
        "n_subjects": int(len(y)),
        "fold_scores": fold_scores,
        "summary": summary,
    }
    out_path = osp.join(args.out_dir, f"{args.algorithm}__local_pcc_combat.json")
