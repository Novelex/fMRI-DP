#!/bin/bash
#SBATCH --job-name=final-grid-combat
...
#SBATCH --array=0-59
#SBATCH --output=/users/3171356m/muhammad/GraSTIACL/nested_cv/logs/final_grid_combat_%A_%a.out
#SBATCH --error=/users/3171356m/muhammad/GraSTIACL/nested_cv/logs/final_grid_combat_%A_%a.err
...
echo "Task ${SLURM_ARRAY_TASK_ID}: config=${CONFIG_IDX} fold=${FOLD} batch_size=${BATCH_SIZE} emb_dim=${EMB_DIM} mij_source=${MIJ_SOURCE} combat=yes"
